...
export const getTodos = async (req: Request, res: Response): Promise<void> => {
    ...
    export const getTodoById = async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            // Jika underfined, berarti todo tidak ditemukan atau bukan milik user ini
            if (!todo) {
            ...
            res.status(500).json({ siccess: false, message: 'Gagal mengambil data.'})
        }
    };
...
export const createTodo = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        res.status(201).json({
            ...
            data: { id: newId, task, id_completed: false }
        });
    } catch (error) {
    ...
    export const updateTodo = async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            //Jika affectedRows > 0, berarti todod tidak ditemukan atau bukan milik user ini
            if (affectedRows === 0) {
                res.status(404).json({ success: false, message: 'Tugas tidak ditemukan!' });
                return;
            }
