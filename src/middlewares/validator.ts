...
export const validateRegister = (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!username || !email || !password) {
        res.status(400).json({ success: false, message: 'Username, email, and password wajib diisi!' });
        return;
    }
    if (!email.includes('@')) {
...
// Validasi untuk update todo - task dan is_completed bolehdikirm bersamaan
export const validateUpdateTodo = (req: Request, res: Response, next: NextFunction): void => {
    ...
    //jika is_completed dikirm, harus berupa boolean
